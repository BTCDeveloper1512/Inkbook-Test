...
/**
 * Depth Carousel from React Bits (MIT) — https://reactbits.dev/components/depth-carousel
 *
 * Copied rather than installed: React Bits is a copy-paste library, there is
 * no package to depend on. GSAP was already a dependency here, so this adds
 * nothing new.
 *
 * One addition to the original: `onActivate`. Clicking a card that is already
 * focused calls it, which is how the portfolio opens the lightbox on the
 * image you're looking at. Clicking any other card still just brings it to
 * the front, exactly as upstream.
 */
...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      el.style.transform = `translate(-50%, -50%) scale(${sc}) translateX(${tx.toFixed(2)}px) translateZ(${tz.toFixed(
        2
      )}px) rotateY(${ry.toFixed(3)}deg)`;
  ...
  const onCardClick = useCallback(
    (index) => {
      ...
      // Addition to upstream: a second click on the front card opens it.
      if (index === focusRef.current) onActivateRef.current?.(index);
  ...
  return (
    ...
    >
      ...
      {showIndicators && count > 1 && (
        <div className="depth-carousel__dots" role="tablist" aria-label="Bilder">
          {data.map((_, i) => (
            <button
              key={i}
              type="button"
