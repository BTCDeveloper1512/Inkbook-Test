            ...
            <>
              ...
              <div className="-mx-2 rounded-2xl overflow-hidden bg-white border border-zinc-100" style={{ height: 320 }}>
                <DepthCarousel
                  ...
                  visibleCards={3}
                  perspective={1100}
                  onActivate={(i) => onOpenImage(artist.portfolio_images, i)}
                />
              </div>
              <p className="text-[11px] font-inter text-zinc-400 mt-2 text-center">
                Ziehen oder Pfeiltasten zum Blättern · Klick auf das vordere Bild öffnet es groß
              </p>
            </>
          )}
        </div>
...
export default function PublicStudioPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      ...
      <AnimatePresence>
        {/* Both children need explicit keys: AnimatePresence tracks its
            children by key, and with two conditional ones it can't tell them
            apart when both are open at once. */}
        {selectedArtist && (
          <ArtistModal
            key="artist"
